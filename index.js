...
function shouldRunAnimations() {
  ...
  const mobileKeywords = ['mobile', 'iphone', 'ipod', 'android', 'blackberry', 'windows phone', 'webos', 'opera mini'];
  const tabletKeywords = ['ipad', 'tablet', 'kindle', 'silk', 'playbook', 'nexus 7', 'nexus 10', 'xoom', 'sch-i800', 'android 3'];
...
if (animationsAllowed) {
  ...
  lenis.on('scroll', ({
    ...
    const nextSection = heroSection === null || heroSection === void 0 ? void 0 : heroSection.nextElementSibling;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    splitTypes.forEach((el, i) => {
      ...
      mainTimeline.fromTo(split.words, {
        opacity: 0.5,
        filter: blurStart
      }, {
        opacity: 1,
    ...
    // --- Underline link animation for .reveal-type .underline-link ---
    splitTypes.forEach((el, i) => {
      const underlineLink = el.querySelector('.underline-link');
      if (underlineLink) {
        // Get the ::after pseudo-element via CSS variable trick
        // We'll animate a real span overlay for better control
        let underlineSpan = underlineLink.querySelector('.underline-anim');
        if (!underlineSpan) {
          underlineSpan = document.createElement('span');
          underlineSpan.className = 'underline-anim';
          underlineLink.appendChild(underlineSpan);
        }
        // Style the underline span to match the ::after
        Object.assign(underlineSpan.style, {
          position: 'absolute',
          left: 0,
          bottom: '-4px',
          height: '4px',
        ...
        // Make sure underlineLink is relative
        underlineLink.style.position = 'relative';
        underlineSpan.style.display = 'block';

        // Animate underline span with ScrollTrigger
        gsap.set(underlineSpan, {
          width: '0%',
          opacity: 0.5,
          filter: blurStart
        });
        const underlineTimeline = gsap.timeline();
        underlineTimeline.to(underlineSpan, {
          width: '100%',
          opacity: 1,
          filter: blurEnd,
          duration: 1.5,
    ...
    function forceCheckAllAnimations() {
      ...
      animeContainers.forEach((container, index) => {
        ...
        if (container && target) {
          ...
          const position = lottieScrollPositions[index] || {
            start: 'center 70%'
          };
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const imageSources = [{
    before: "qw-before.webp",
    after: "qw.webp"
  }, {
    before: "coral-before.webp",
    after: "coral.webp"
  }, {
    before: "pb-before.webp",
    after: "pb.webp"
  }, {
    before: "kati-before.webp",
    after: "kati.webp"
  }, {
    before: "sara-before.webp",
    after: "sara.webp"
  }];
